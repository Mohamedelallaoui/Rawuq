...
export default function MegaMenu() {
  ...
  /* ── MOBILE PANEL (rendered via portal to document.body) ──────── */
  const mobilePanel = mounted && mobileOpen ? createPortal(
    <div style={{
      position: 'fixed', inset: 0,
      // Sits above everything — no parent stacking context can trap it
      zIndex: 99999,
      background: bg,
      overflowY: 'auto',
      direction: 'rtl',
      ...
      <div style={{
        ...
        <button
          ...
          style={{
            ...
            color: subTextColor, fontSize: 20, lineHeight: 1, borderRadius: 6,
          }}
        >✕</button>
      </div>
    ...
    document.body   // ← renders directly on body, outside any stacking context
  ) : null
        ...
        >
          <div style={{ maxWidth: 900, margin: '0 auto', padding: '0 24px', direction: 'rtl' }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '2px 0' }}>
              <Link href={`/category/${encodeURIComponent(activeCat.name)}`} style={{
                fontSize: 26, fontWeight: 700, color: textColor,
                textDecoration: 'none', padding: '4px 0',
                width: '100%', display: 'block',
              }}>كل {activeCat.name}</Link>
              {activeCat.sub.map(sub => (
                <Link key={sub} href={`/category/${encodeURIComponent(sub)}`} style={{
